...
class RecAuditRepository extends BaseRepository {
  async insert(
    entityType: string,
    entityId: number,
    action: string,
...
/**
 * Shared audit trail for every internal-recruitment entity. Both recruitment
 * work streams import this; it owns the rec_audit_logs table alone.
 */
export class RecruitmentAuditService {
  private repo = new RecAuditRepository();
  ...
  async record(
    ...
    try {
      ...
      console.error(`recruitment audit write failed for ${entityType}#${entityId} ${action}:`, err);
    }
  }
