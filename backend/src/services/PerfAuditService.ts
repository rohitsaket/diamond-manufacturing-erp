...
import { PerfActionContext, PerfAuditLogResponse } from '../types/performance';

class PerfAuditRepository extends BaseRepository {
  async insert(
    entityType: string,
    entityId: number,
    action: string,
...
/**
 * Shared audit trail for every performance entity. Both performance work
 * streams import this; it owns the perf_audit_logs table alone.
 */
export class PerfAuditService {
  private repo = new PerfAuditRepository();
  ...
  async record(
    ...
    try {
      ...
      console.error(`perf audit write failed for ${entityType}#${entityId} ${action}:`, err);
    }
  }

  async list(filters: { entityType?: string; entityId?: number; limit?: number }): Promise<PerfAuditLogResponse[]> {
